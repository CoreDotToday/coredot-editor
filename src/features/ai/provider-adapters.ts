...
function createOpenAiProvider(settings?: AiProviderSettings): AiProvider {
  const model = settings?.aiModel ?? process.env.OPENAI_MODEL ?? getAiProviderDefinition("openai").defaultModel;
...
function createCoreDotProvider(settings?: AiProviderSettings): AiProvider {
  ...
  const model = settings?.aiModel ?? process.env.COREDOT_MODEL ?? getAiProviderDefinition("coredot").defaultModel;
...
function createCoreDotAnthropicProvider(settings?: AiProviderSettings): AiProvider {
  ...
  const definition = getAiProviderDefinition("anthropic");
  const model = settings?.aiModel ?? process.env.COREDOT_ANTHROPIC_MODEL ?? definition.defaultModel;
  const maxOutputTokens =
    settings?.aiMaxCompletionTokens ??
...
function createCoreDotGeminiProvider(settings?: AiProviderSettings): AiProvider {
  ...
  const definition = getAiProviderDefinition("gemini");
  const model = settings?.aiModel ?? process.env.COREDOT_GEMINI_MODEL ?? definition.defaultModel;
  const maxOutputTokens =
    settings?.aiMaxCompletionTokens ??
    readOptionalPositiveInteger(process.env.COREDOT_MAX_COMPLETION_TOKENS) ??
    definition.defaultMaxCompletionTokens!;
...
function createStubAiProvider(): AiProvider {
  ...
  return {
    ...
    name: "stub",
    model: getAiProviderDefinition("stub").defaultModel,
    async generateText(input) {

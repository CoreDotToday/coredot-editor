...
- Left: live document outline, prompt templates, template variables, and AI run history
...
AI context transparency is handled by `src/features/ai/ai-context-snapshot.ts` and `AiContextInspector`. The snapshot records the active document title and character counts, selected template, variable names and redacted sensitive variable values, selected text metadata when available, and head/tail-truncated document text. The inspector exposes this as a copyable debug payload without storing provider secrets in browser state.

...
The browser barrier is `{ generation, stateVector }`, where `stateVector` is canonical unpadded base64url. `flushPendingUpdates()` completes only after initial sync, a writable synchronized transport, zero provider-side unsynchronized changes, and durable acknowledgement of every local update. It is abortable and time-bounded. A read-only, destroyed, fatally failed, disconnected, timed-out, or aborted session fails before an AI provider call, Proposal command, preview, or export begins; none of these paths falls back to the browser draft.

For an initialized collaborative document, `collaborationBarrier` is required by both AI routes. The server verifies the same generation and proves that the exact durable snapshot contains every client clock represented by the barrier state vector. The submitted `documentText`, title, body JSON, and metadata are never used as collaborative authority. A barrier on a legacy document, a missing barrier on an initialized document, a malformed/non-canonical vector, or a vector not contained in the server snapshot returns `409 collaboration_snapshot_conflict`; a retryable storage failure returns `503 collaboration_snapshot_unavailable`.

The AI Run and `collaboration_ai_run_snapshots` identity are inserted in one transaction. The durable snapshot identity contains document, generation, head, state vector, schema fingerprint, and a SHA-256 hash of the canonical title/body/metadata materialization. Completed idempotent replays return that stored result even if the live head later advances. Reusing the key with a different barrier or operation payload returns `409` rather than rebinding the run to a new snapshot. Collaborative rewrite additionally requires the exact ProseMirror `selectionRange`; the range and selected text must match the stored checkpoint before provider execution.

Legacy AI claims and finalization both recheck in their SQL transaction that the document still has no current collaboration generation. If collaboration initializes while a provider is running, finalization rolls back before storing any anchorless Proposal and the execution-token failure path leaves the run safely failed. Initialization atomically changes pre-existing pending legacy Proposals to `rejected`; accepted/rejected history is retained, but no numeric-position Proposal remains apparently applicable after the authority cutover. The generic status-reset transaction cannot revive one of those rejected legacy Proposals after initialization: `rejected -> pending` requires a Proposal anchor for the exact current collaboration generation, otherwise it returns `409 collaboration_anchor_required`. Legacy documents and exactly anchored collaborative Proposals retain their normal reset behavior.
...
The Proposal status transition, exact Yjs update, action, document projection, Document Change, Proposal links, command-delivery job, and any workflow-notification job commit atomically. The sidecar command-delivery worker later publishes the exact stored generation/sequence/checksum update; it never regenerates a command against the current head. An unloaded room counts as delivered because its next connection reloads the durable state. Delivery retries are bounded and persist only a stable failure category. An exact semantic command replay returns the existing result and explicitly re-arms an exhausted matching delivery; a different command fingerprint fails idempotency. Pending delivery rows restrict deletion of their action and update, so checkpoint compaction cannot silently discard an update that still requires publication.

DOCX preview and export follow the same authority rule. The browser first completes the durability barrier. The initialized-document routes then ignore the submitted draft and title, materialize the exact canonical Y.Doc, and return generation, head, schema fingerprint, and canonical content-hash diagnostics in preview/error JSON or successful export headers. Legacy documents retain the submitted-draft contract. Reference documents are intentionally different: AI may read their fenced SQL projection, and hydration carries generation and `projectedSeq` diagnostics so projection lag is observable without treating the reference as an exact live snapshot.
...
Archiving commits the document status change and an exact-current-generation room-closure job in one transaction. The archive service accepts an optional local gateway: the normal Next.js process supplies none, leaves the new job due immediately with zero attempts and no failure category, and preserves the entire bounded retry budget for the sidecar. Immediate delivery occurs only when a real gateway is present; absence of a gateway is not recorded as a failed attempt. The general document repository exposes no archive writer, so production callers cannot bypass the transactional closure outbox with a status-only update. A sidecar reconciler claims due jobs in bounded batches and passes each job's stored generation to a dedicated archive-room gateway. That gateway builds and closes the exact local Hocuspocus room idempotently; it never reloads or substitutes whichever generation happens to be current during a later retry. Failures use capped exponential backoff, and permanently exhausted work remains available for operator inspection after the bounded attempt limit. Shutdown first stops and awaits this worker, then drains the sidecar and closes the database. An archive response distinguishes `not_required`, `delivered`, and durable `pending` closure without exposing the room name or generation. A client may copy unsynchronized local content but cannot continue writing to the archived canonical document.

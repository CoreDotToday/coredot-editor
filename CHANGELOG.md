...
First public v1 release of Coredot Editor as an open-source AI document editor starter.

### Added

- Added a Korean-first editor UI with English language switching.
- Added Core.Today LLM proxy support for OpenAI-compatible, Anthropic, and Gemini routes.
- Added an LLM settings dialog for provider, model, Base URL, token limit, reasoning effort, and connection checks.
- Added a right-side AI workspace with review, conversation, and change-history tabs.
- Added a bottom AI command bar for selection, current-block, and whole-document edits.
- Added editor and Source views, outline navigation, find and replace, a command palette, and an inspectable AI context panel.
- Added Proposal workflows with pending, accept, reject, bulk actions, document focus, redline preview, and local undo.
- Added a contract review template and prompt guidance for structured legal-style findings.
- Added scope-aware quick action presets to the bottom AI command bar.
- Added a slash command menu, block gutter controls, drag indicators, block insertion, duplication, deletion, indentation, outdent, and list-to-paragraph conversion flows.
- Added Notion-style `Cmd/Ctrl+A` behavior for the current block and then the whole document.
- Added Markdown paste support for common block structures including pipe tables.
...
- Added an optional Docker verification stack for PostgreSQL/pgvector and Chroma retrieval experiments without changing the default SQLite application database.
- Added responsive layout tests for narrow editor viewports.
...
- Hardened Core.Today Base URL handling so browser and database settings cannot redirect server-side API keys.
...
- Patched the Babel toolchain advisory and removed deprecated Clerk route-matcher authentication gates in favor of resource-level checks.
